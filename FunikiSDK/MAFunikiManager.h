...
@optional
/**
 *  startSelectingDeviceを呼び出した後、発見した雰囲気メガネが配列 [MADiscoverdPeripheral] で渡される。
 *
 *  @param manager     メソッドの呼び出し元
 *  @param peripherals MADiscoverdPeripheralの配列。配列はrssi順にソートされている。ただし、すでにiOSと雰囲気メガネ間の接続が確立されている場合、rssiは0になる。
 */
- (void)funikiManager:(MAFunikiManager *)manager didUpdateDiscoverdPeripherals:(NSArray *)peripherals;
 ...
 */
@property (nonatomic, readonly) NSArray *discoverdPeripherals;
...
/**
 *  雰囲気メガネを探索/登録する。雰囲気メガネを発見すると、delegateのfunikiManager:didUpdateDiscoverdPeripherals:が呼び出される。このメソッドを呼び出すと、雰囲気メガネへの自動接続がキャンセルされる。
 */
